#SILAHKAN RECODE MASTAH
#RECODE TIDAK MEMBUAT MU PRO
#MELAINKAN MEMBUAT MU NOOB

#MR.K7C8NG

#-----------------

red='\033[1;31m'

rset='\033[0m'

grn='\033[1;32m'

ylo='\033[1;33m'

blue='\033[1;34m'

cyan='\033[1;36m'

pink='\033[1;35m'

#-----------------

# scripting starts

clear

echo '

                

                         __________

                      .~#########%%;~.

                     /############%%;`\

                    /######/~\/~\%%;,;,\

                   |#######\    /;;;;.,.|

                   |#########\/%;;;;;.,.|

          XX       |##/~~\####%;;;/~~\;,|       XX

        XX..X      |#|  o  \##%;/  o  |.|      X..XX

      XX.....X     |##\____/##%;\____/.,|     X.....XX

 XXXXX.....XX      \#########/\;;;;;;,, /      XX.....XXXXX

X |......XX%,.@      \######/%;\;;;;, /      @#%,XX......| X

X |.....X  @#%,.@     |######%%;;;;,.|     @#%,.@  X.....| X

X  \...X     @#%,.@   ----------------    @ @ 00 0 xxxxxxxxx

 X# \.X        @#%,.@   Infect v1.1      @#%,.@        

                @#%,.@              @#%,.@          

                  @#%,.@          @#%,.@            

                     @#%,.@      @#%,.@             

                       @#%.,@  @#%,.@              

                         MR.K7C8NG

'|lolcat

echo " " 

echo -e "$red                      ▶ Coded by$grn MR.K7C8NG$red ◀$rset"

echo -e "$red                        ⫸$ylo @pranata_pasha$red ⫷$rset"

echo -e "$red                 ▶$grn INDONESIA CYBER ERROR SYSTEM$red ◀$rset"

echo 

# Functions

echo -e "$red            [Menginfeksi mereka dengan kesenangan]$rset"

echo " "

echo " "

echo -e "$grn              ＞＞＞＞>>>＞$ylo [Options]$grn ＜<<<＜＜＜＜$rset"

echo " "

echo " "

echo -e "$red                        ➡$cyan [1️⃣] Infect now"

echo -e "$red                        ➡$cyan [2️⃣] Save now"

echo -e "$red                        ➡️$cyan [3️⃣] Update script"

echo -e "$red                        ➡$cyan [4️⃣] Exit"

echo " "

echo " "

echo -e "$grn               >＞＞＞＞＞$ylo [SELECT]$grn ＜＜＜＜＜<$rset"

echo " "

echo -e "$cyan                   [[[$ylo Select any option$cyan]]]$rset"

echo " "

read n

case "$n" in

1)echo " "

echo " "

echo " "

echo -e "$red               Kamu akan$grn Infect$red Now$rset"

echo " "

echo -e "$grn                  Press$ylo ENTER$red to continue$rset"

echo " "

read hulke

cd $HOME/Infect/lol

echo " "

echo -e "$ylo     Untuk mengakhiri proses klik$red 't'$ylo atau untuk melanjutkan klik$grn ENTER$rset" 

read choice

if [ $choice = 't' ] ; then

echo -e "$red                    Apakah kamu yakin tekan$grn ENTER$red to exit$rset"

read hulkee

cd $HOME/Infect/

bash infect.sh

else

bash sysup.sh

echo " "

echo " "

echo -e "$grn ＞＞＞＞＞＞＞＞＞$ylo [EXIT]$grn ＜＜＜＜＜＜＜＜＜$rset"

echo " "

echo -e "$cyan            click$grn ENTER$cyan untuk kembali ke menu infeksi$rset"

echo " "

read punch

fi

cd $HOME/Infect/

bash infect.sh ;;

2)echo " "

echo " "

echo " "

echo -e "$red                kamu akan$red Seve Korban$rset "

echo " "

echo -e "$grn                  Press$ylo ENTER$grn to continue$rset"

echo " "

read nitro

cd $HOME/Infect/lol

echo " "

echo -e "$ylo         Untuk mengakhiri proses klik$red 't' $ylo atau untuk melanjutkan klik$grn ENTER$rset"

read choice

if [ $choice = 't' ] ; then

echo -e "$red                   Apakah kamu yakin tekan$grn ENTER$red to exit$rset"

read nitroo

cd $HOME/Infect

bash infect.sh

else

bash save.sh

echo " "

echo " "

echo -e "$grn ＞＞＞＞＞＞＞$ylo [EXIT]$grn ＜＜＜＜＜＜＜＜＜$rset"

echo " "

echo -e "$cyan             click$grn ENTER$cyan untuk kembali ke menu infeksi$rset"

echo " "

read punch

fi

cd $HOME/Infect

bash infect.sh ;;

3)echo " "

echo " "

echo " "

echo -e "$red             Kamu akan$grn Perbarui$red Script$rset "

echo " "

echo -e "$grn                 Press$ylo ENTER$grn to continue$rset"

echo " "

read mon

cd $HOME/Infect/lol

echo " "

echo -e "$ylo         Untuk mengakhiri proses klik$red 't' $ylo atau untuk melanjutkan klik$grn ENTER$rset"

read choice

if [ $choice = 't' ] ; then

echo -e "$red                 Apakah kamu yakin tekan$grn ENTER$red atau exit$rset"

read monn

cd $HOME/Infect

bash infect.sh

else

bash update.sh

echo " "

echo " "

echo -e "$grn ＞＞＞＞＞＞＞＞$ylo [EXIT]$grn ＜＜＜＜＜＜＜＜＜$rset"

echo " "

echo -e "$cyan        click$grn ENTER$cyan to go back into infect  menu$rset"

echo " "

read punch

fi

cd $HOME/Infect

bash infect.sh ;;

4)

sleep 2.0

clear

echo " "

echo -e "$grn

     ____  _  _  ____    ____  _  _  ___

    (  _ \( \/ )( ___)  (  _ \( \/ )( ___)

     ) _ < \  /  )__)    ) _ < \  /  )__)

    (____/ (__) (____)  (____/ (__) (____)...$cyan semoga harimu menyenangkan$rset"

echo " "

echo " "

esac

